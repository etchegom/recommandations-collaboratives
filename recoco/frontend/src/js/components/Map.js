...
function Map() {
  return {
    ...
    async init() {
      ...
      //Center Map
      Map.panTo(new L.LatLng(46.51, 1.2));
    },
  };
}
...
function initMap() {
  ...
  const map = L.map('map').setView([48.51, 10.2], 5);
...
function initMapLayers(map, projects) {
  ...
  projects.forEach((project) => {
    if (project?.commune?.latitude && project?.commune?.longitude) {
      let marker = L.marker(
        [project.commune.latitude, project.commune.longitude],
...
function createMarkerIcon(project) {
  return L.divIcon({
    className: 'map-marker ' + statusToColorClass(project.status),
  });
}
